...
> 文档状态：执行中（阶段 2：**核心完成，发布收口中**）
> 日期：2026-09-23；状态更新：2026-09-23
> 适用仓库：AutoForceAI、Genesis_CRM
> 原则：阶段 5 继续保留到最后；本计划只完成阶段 2 的交付收口，并为阶段 4 建立清晰起点。
> 当前进度：§3 五项 P0 ✅ 全部完成（`codex/phase2-release-ready`，测试 75/75）；
> 发布收口（历史净化、凭证轮换、worker 并发保护、CI 门禁、运维手册、PG 演练）进行中；
> 兼容性记录：docs/integration/COMPATIBILITY.md；运维手册：docs/PHASE2_OPS_RUNBOOK.md。

## 1. 执行结论

阶段 2 的业务核心目标已经达成，但尚未达到“可合并、可发布、可长期运行”的工程完成标准。

下一步按以下顺序推进：

1. 完成阶段 2.9 的五项 P0 修复。
2. 完成双仓库合并、推送、版本标记和 CI 门禁。
3. 用一个真实 Genesis project 做 24 小时小规模试运行。
4. 通过试运行后正式关闭阶段 2。
5. 重新核对阶段 4 与 Genesis 现有 Agent 能力的重叠，优先启动 4.3“AI 报价单 + PDF”。
...
Genesis Integration API v1 新增兼容性端点或发布 v1.1：
...
| 批次 | 内容 | 预估 |
|---|---|---:|
| A | token 加密、项目绑定保护、启用门禁 | 1–2 开发日 |
| B | SSRF 防护、Alembic migration | 1–2 开发日 |
| C | worker 进程保护、CI、双端 build | 1 开发日 |
| D | 分支合并、版本标记、试运行准备 | 0.5–1 开发日 |
| E | 24 小时试运行 | 1 个自然日 |
| F | 阶段 4.0 验收与 4.3 详细设计 | 0.5–1 开发日 |
...
- [ ] 3.1–3.5 五项 P0 全部完成。
- [ ] 双端测试、类型检查和 production build 全部通过。
- [ ] 双仓库代码已经进入可追踪的远程分支或 main。
- [ ] OpenAPI 契约和双方 commit/tag 已互相记录。
- [ ] 24 小时试运行通过。
- [ ] 没有明文 service token。
- [ ] 没有跨项目统计、投递或回流污染。
- [ ] PostgreSQL migration 经过演练。
- [ ] 阶段 2 运维手册说明凭证签发、轮换、撤销、死信重投和项目绑定重置。

满足后将阶段 2 标记为：`✅ 正式完成`；Wave E 以独立 P3 backlog 保留，不影响阶段 2 核心关闭。

## 10. 下一轮直接执行清单

下一轮开发按以下顺序，不再重新讨论总体架构：

1. [ ] 建立 `codex/phase2-closeout` 分支，保护当前工作区改动。
2. [ ] 实现 CRM token 加密与迁移测试。
3. [ ] 实现项目绑定不可直接切换与 reset-binding。
4. [ ] 实现 CRM 地址 allowlist 与 SSRF 测试。
5. [ ] 引入 Alembic，完成阶段 2 schema migration。
6. [ ] 拆分保存、测试、启用三个动作。
7. [ ] 增加 outcome poller 租约或明确单 worker 配置。
8. [ ] 增加双仓库 CI 门禁并跑 production build。
9. [ ] 合并/推送/打标签，执行 24 小时试运行。
10. [ ] 关闭阶段 2，启动阶段 4.0 与 4.3。
