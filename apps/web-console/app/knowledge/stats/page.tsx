  ...
  return (
    <div className="min-h-screen bg-bg text-text p-8">
      ...
      <div className="space-y-2">
        {kbs.map(kb=>(
          <div key={kb.id} className="p-3 rounded border border-separator flex justify-between">
            <span>{kb.name}{kb.is_public ? " · 公开" : ""}</span>
            <span className="text-text-secondary text-sm">{kb.doc_count} 文档 · {kb.chunk_count} 切片</span>
          </div>
        ))}
        {!kbs.length && <p className="text-text-secondary">暂无知识库。请先在「知识文档」创建并上传资料。</p>}
      </div>
    </div>
  );
}
