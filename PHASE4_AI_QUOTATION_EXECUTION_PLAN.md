...
阶段 4 首个新功能确定为 **4.3 AI 报价单 + PDF**。本轮只冻结边界和契约，
后续实现不得改变阶段 2 的 v1.0 基础契约哈希。
...
- 镜像扩展契约并固定哈希；
- Genesis 契约/业务测试与 AutoForceAI 解析测试；
- 一条真实已同步客户 E2E；
- 双仓库 CI、敏感信息扫描和兼容性记录更新。

## 12. 完成定义

- [ ] 未确认前 Genesis 中没有新报价；
- [ ] 确认后只创建一份 draft，重复请求返回同一报价；
- [ ] 请求无法传入或覆盖行金额、总额和状态；
- [ ] Genesis 重算金额并返回权威详情；
- [ ] 报价进入正确客户的 Timeline；
- [ ] 跨项目、缺 scope、撤销凭证和越权读取均被拒绝；
- [ ] PDF 与数据库字段一致，中文英文可读，draft 有水印；
- [ ] 同一报价版本 PDF 内容和 ETag 稳定；
- [ ] AutoForceAI 展示来源、缺失信息和风险，不编造关键字段；
