...
| 阶段 5 | 🟡 工程实现已合并，候选验收中 | H-09–H-13 双仓主线包含部署/观测/安全/备份工件；真实 HTTPS、全新卷/恢复与 24 小时试运行未验证 |

`a55741a` 和 `e9b7c64` 是换电脑时必须至少包含的基线。上方“当前权威状态”记录的是 **2026-09-24 制定时快照**，并非 H-14 当前 HEAD；2026-09-26 H-14 开工时 AutoForceAI `origin/main@7b29a04`、Genesis_CRM `origin/main@2213b45`，H-13 已分别合入。本文合并后还会产生新 SHA，因此最终判断应使用 `git log origin/main`，不能要求 HEAD 永远等于上述旧 SHA。H-14 最新证据见 `docs/integration/COMPATIBILITY.md` 与 `deploy/H14_RELEASE_CHECKLIST.md`。
...
| H-14 | 全量回归、文档收口与候选发布 | 两仓库 | P0 | 🟡 收口 PR 中：本地全量门禁 skip=0，待两仓远程 CI、远程旧分支清理及候选 tag 前置复核 |
