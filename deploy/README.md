...
- Next.js 浏览器 API 基址须与前端同源。容器构建固定 `NEXT_PUBLIC_API_URL` 为空，生产 Nginx 把 `/api/`、`/auth/`、`/uploads/`、`/widget/` 等转发 backend；Next 服务端 `/api/` rewrite 的内部上游 `INTERNAL_API_URL=http://backend:8010`。Genesis Vite build-time `VITE_API_BASE_URL=/api`，Genesis web Nginx 转发 `/api/` 到 `genesis-server:5000`。不使用子路径承载另一前端；双域名直接分配各自虚拟主机。
